    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      ImageSource.forEach(async (element) => {
        for (let i = 0; i < element.cityImg.length; i++) {
          element.cityImg[i].cityCenter = await convertNametoLongLat(
            element.cityImg[i].cityName,
          )
          new mapboxgl.Marker({
            color: '#00FFFF',
          })
            ...
            .addEventListener('click', async (e) => {
              let postedMarkersData = await postMarkerInfo(
                'markers',
                element.cityImg[i].cityName,
                element.cityImg[i].cityCenter,
                element.cityImg[i].cityId,
                element.cityImg[i].citySrc,
              )
...
const thisItemValue = (itemValue) => {
  console.log(itemValue.place_name)
  return itemValue.place_name
}
      ...
      let thisMap = new mapboxgl.Map({
        ...
        let thisGeocoder = new mapGeocoder({
          ...
          thisMarker.getElement().addEventListener('click', async (e) => {
            ...
            let postedData = await postMarkerInfo(
              'marker',
...
export function postMarkerInfo(
  thisRoute,
  markerName,
  markerCenter,
  markerId,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let thisResponse = await fetch(
        `http://localhost:5000/api/map/${thisRoute}/data`,
        {
